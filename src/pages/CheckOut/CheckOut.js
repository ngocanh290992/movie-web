...
export default function CheckoutTab(props) {
    ...
    const operations = <Fragment>
        {!_.isEmpty(userLogin) ? <div className='flex flex-row items-center'>
            <button className='flex flex-row items-center' onClick={() => {
                history.push(`/profile/${userLogin.taiKhoan}`)
            }}>
                <div style={{ width: 50, height: 50, display: 'flex', justifyContent: 'center', alignItems: 'center' }} className="text-2xl mr-2 rounded-full bg-red-500">
            ...
            }} className="ml-5 text-dark font-semibold">
                Đăng xuất
            </button>
        </div> : ''}
    ...
    const items = [
        { label: '01 CHỌN GHẾ & THANH TOÁN', key: '1', children: <CheckOut {...props} /> }, // remember to pass the key prop
        { label: '02 KẾT QUẢ ĐẶT VÉ', key: '2', children: <KetQuaDatVe {...props} /> },
        { label: <div className="text-center" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><NavLink to="/"><HomeOutlined style={{ marginLeft: 10, fontSize: 25 }} /></NavLink></div>, key: '3' }
    ];
    return <div className="p-5">
        <Tabs tabBarExtraContent={operations} activeKey={tabActive} items={items} onChange={(key) => {
            console.log({ key })
            dispatch({
                type: CHANGE_TAB_ACTIVE,
                number: key.toString()
